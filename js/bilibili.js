...
class Spider:
    ...
    def playerContent(self, flag, ids, vipFlags=None):
        ...
        parts = raw.split("$")
        raw_arg = parts[1] if len(parts)>=2 else raw
        sub_parts = raw_arg.split("_")
        aid = cid = eid = bvid = ""
        if len(sub_parts) >= 4:
            aid, cid, eid, bvid = sub_parts[0], sub_parts[1], sub_parts[2], sub_parts[3]
        elif len(sub_parts) == 3:
            aid, cid, eid = sub_parts[0], sub_parts[1], sub_parts[2]
        elif len(sub_parts) == 2:
            eid, cid = sub_parts[0], sub_parts[1]
        if cid:
        ...
        def _try_qn(qn):
            if bvid and bvid != "0":
                api_url = "{}/x/player/playurl?bvid={}&cid={}&qn={}&fnver=0&fnval=4048&fourk=1".format(API, bvid, cid, qn)
            elif aid and aid != "0":
                api_url = "{}/x/player/playurl?avid={}&cid={}&qn={}&fnver=0&fnval=4048&fourk=1".format(API, aid, cid, qn)
            elif eid and eid != "0":
                api_url = "{}/pgc/player/web/playurl?cid={}&qn={}&fnver=0&fnval=4048&fourk=1&ep_id={}".format(API, cid, qn, eid)
            else:
                api_url = "{}/x/player/playurl?cid={}&qn={}&fnver=0&fnval=4048&fourk=1".format(API, cid, qn)
            resp = self.http.get(api_url, timeout=10)
            data = resp.json()
            if data.get("code") != 0:
                return None
            ...
            if not videos_sorted or not audios_sorted:
                return None
            top_video = videos_sorted[0]
            top_audio = audios_sorted[0]
            v_url = self._pick_url_for_mpd(top_video)
            a_url = self._pick_url_for_mpd(top_audio, is_audio=True)
            ...
            try:
                ...
                vp = urlparse(v_url)
                au = urlparse(a_url)
                mcdn_domain = au.netloc if "mcdn" in au.netloc else None
                if mcdn_domain and "mcdn" not in vp.netloc:
                    v_url = urlunparse(vp._replace(netloc=mcdn_domain, path=vp.path))
            except Exception:
                pass
            seg_v = top_video.get("SegmentBase", {}) or top_video.get("segment_base", {})
            init_v = seg_v.get("Initialization", "") or seg_v.get("initialization", "")
            idx_v = seg_v.get("indexRange", "") or seg_v.get("index_range", "")
            seg_xml_v = ""
            if init_v or idx_v:
                seg_xml_v = '<SegmentBase indexRange="{}"><Initialization range="{}" /></SegmentBase>'.format(idx_v, init_v)
            seg_a = top_audio.get("SegmentBase", {}) or top_audio.get("segment_base", {})
            init_a = seg_a.get("Initialization", "") or seg_a.get("initialization", "")
            idx_a = seg_a.get("indexRange", "") or seg_a.get("index_range", "")
            seg_xml_a = ""
            if init_a or idx_a:
                seg_xml_a = '<SegmentBase indexRange="{}"><Initialization range="{}" /></SegmentBase>'.format(idx_a, init_a)
            ...
            mpd += '<Representation id="video_0" bandwidth="{}" codecs="{}" width="{}" height="{}" frameRate="{}" sar="1:1"><BaseURL>{}</BaseURL>{}</Representation>'.format(
                top_video.get("bandwidth", 0), top_video.get("codecs", "avc1.64001F"),
                top_video.get("width", 1920), top_video.get("height", 1080), top_video.get("frameRate", "30"),
                html.escape(v_url), seg_xml_v)
            mpd += '</AdaptationSet>'
            mpd += '<AdaptationSet mimeType="audio/mp4" startWithSAP="1" subsegmentAlignment="true">'
            mpd += '<Representation id="audio_0" bandwidth="{}" codecs="{}"><BaseURL>{}</BaseURL>{}</Representation>'.format(
                top_audio.get("bandwidth", 128000), top_audio.get("codecs", "mp4a.40.2"),
                html.escape(a_url), seg_xml_a)
            mpd += '</AdaptationSet>'
            mpd += '</Period>'
            mpd += '</MPD>'
