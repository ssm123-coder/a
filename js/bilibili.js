var rule = {
    ...
    headers:{
        ...
        "Referer": "https://www.bilibili.com",
         "Cookie": "DedeUserID=3494349660752443; DedeUserID__ckMd5=74d3f68f652c20c5; SESSDATA=ed91686f,1799832204,ad34a*72CjDPtz5qmaZnTTuGE0qJ6rfSRCx3LbbYcjrwpRQbGHGSU9jS4BjIjVvrbN9J_2e5UaUSVkIwM3lGbmJIVWNvNDZ0cUpLMXhBaWlXeTVfOHFpeDlreDNOc3dadHZ3NnZBUkU3Sl92bU9XLVA0ZjZ0ekd6U3N6N2ZIaEp1cWVOUGRmeHppWGVySFVnIIEC; bili_jct=83142548b194c9db766c8d900f8394a1;"
    },
    tab_order:['恒轩'],
    timeout:5000,
    ...
    const globalHeaders = rule.headers;
    const commonHeader = {
        ...globalHeaders,
    ...
    if (/^http/.test(input)) {
        ...
        let cidMatch = input.match(/cid=(\d+)/) || input.match(/cid%3D(\d+)/); 
        let cid = cidMatch ? cidMatch[1] : "";
        let danmaku = cid ? 'https://api.bilibili.com/x/v1/dm/list.so?oid=' + cid : "";
        
        
        for(let i=0; i<parserUrls.length; i++){
            try {
                let parserUrl = parserUrls[i] + encodeURIComponent(input);
                let html = request(parserUrl, {header: commonHeader}); 
                let jRoot = JSON.parse(html);
                playUrl = jRoot["url"] || jRoot["playUrl"] || jRoot["data"];
                if (playUrl) break;
            } catch (e) {
                print("解析接口" + (i+1) + "失败: " + e.message);
                continue;
            }
        }
        ...
        if (playUrl) {
            result["parse"] = 0;
            result["url"] = playUrl;
            result["header"] = commonHeader;
            result["danmaku"] = danmaku;
            result["danmakuOpen"] = true; 
            result["danmakuType"] = "bilibili"; 
            result["danmakuHeader"] = commonHeader; 
            input = result;
        } else {
            print("所有解析接口均失败：未返回播放地址");
            input = "";
        }
    } 
            ...
            let html = request(url, {header: commonHeader});
            let jRoot = JSON.parse(html);
