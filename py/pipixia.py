...
# 内嵌线路排序函数（无外部依赖）
def sort_play_line(source_list):
    def get_priority(show_name):
        ...
        if '专线' in name:
            return 7
        return 8
    return sorted(source_list, key=lambda x: get_priority(x["show"]))
...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        for i in data3['vod_play_list']:
            ...
            show_name = i['player_info']['show']
            play_url = play_url.rstrip('#')
            temp_lines.append({"show": show_name, "urls": play_url})
        
        # 执行线路排序
        temp_lines = sort_play_line(temp_lines)
        
        # 还原为原拼接格式（原有逻辑、字符分隔完全不变）
        vod_play_from = ''
        vod_play_url = ''
        for item in temp_lines:
            vod_play_from += item['show'] + '$$$'
            vod_play_url += item['urls'] + '$$$'
